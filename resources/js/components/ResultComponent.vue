            ...
            <div>
                        ...
                        <span id="fsize">
                          <h4 class="result-size" v-if="container.showSelectedSizeSlider">
                            ...
                            <span class="variant_title" :data-variant="row.id">
                              <span
                                v-if="
                                  row.option1.toUpperCase().substring(0, 2) == 'XL' ||
                                  row.option1.toUpperCase().substring(0, 2) == 'XS'
                                "
                              >
                              ...
                              <span
                                ...
                                "
                                >{{ row.option1.toUpperCase().charAt(0) }}</span
                              ></span
                            >
                          </h4>
                        </span>
              ...
              >
                <span v-if="!container.is_loading"
                  >Fit Size:<strong>{{ row.desc_title }}</strong></span
                >
              </p>
...
export default {
  ...
  methods: {
      ...
      axios.get(this.$appUrl + "/api/get-attrbutes/" + this.product.id).then((res) => {
        ...
        {
            this.form.bodyMeasure[i]=localStorage.getItem(res.data[i].name.toLowerCase())
        }
        this.form.conversionCount = this.product.id;
        if(localStorage.getItem("FORM__")==null)
        {
             localStorage.setItem("FORM__",JSON.stringify(this.form))
        }
   ...
   this.getAttributes();
                ...
                if (i > this.sizeIndex && i < this.product.variants.length) {
                  ...
                  if (
                    ...
                    this.product.variants.indexOf(
                      this.product.variants[this.product.variants.length - 3]
                    )
                  ) {
                  ...
                  if (
                    i ==
                    this.product.variants.indexOf(
                      this.product.variants[this.product.variants.length - 2]
                    )
                  ) {
                    this.product.variants[i].desc_title = "Relaxed";
                  }
    ...
    getProductDetails: function () {
      ...
      axios.post(this.$appUrl + "/api/size-recommend/",this.formLocal ).then((res) => {
